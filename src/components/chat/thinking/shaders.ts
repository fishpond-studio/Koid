/**
 * WebGL 着色器（移植自 citation/claude-range-slider-main）。
 * 与原版的差异：火焰配色不再硬编码紫色，改为 u_cdeep / u_cmain / u_chot
 * 三个 uniform，由宿主组件按当前主题 --primary 动态计算注入。
 */
...
export const FRAG_SIM = `#version 300 es
  ...
  uniform vec3 u_cdeep, u_cmain, u_chot;
  ...
  void main(){
    ...
    float t=u_time;
    float cellDelay = h * 0.5;
    float cellAge   = max(u_elapsed - cellDelay, 0.0);
    float ignited   = step(0.001, cellAge);
    float cellSpd   = 0.85 + h * 0.30;
    // smoothstep 蔓延：起速为零，无开局突进（ease-out 会在 t=0 处速度最大）
    float ex    = clamp(cellAge / 2.0, 0.0, 1.0);
    float eased = ex * ex * (3.0 - 2.0 * ex);
    ...
    float es = mix(0.3, 0.9, min(u_elapsed / 1.2, 1.0));
    ...
    col *= cell;
    col *= fade_mask;
    fc = vec4(min(decay + col * 1.35, vec3(1.6)), 1.0);
  }
`
...
export const FRAG_COMP = `#version 300 es
  ...
  void main(){
    ...
    fc=vec4(1.0-exp(-(s+g*1.2+s*g*0.35)*1.45),1.0);
  }
`
